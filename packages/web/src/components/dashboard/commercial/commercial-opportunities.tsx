...
import type { Client, CommercialOpportunity, CommercialOpportunityStage, CommercialOpportunityStageEvent, DeepManager, Product } from "@/lib/types/database";
...
export function CommercialOpportunities({ opportunities, events, clients, products, managers, canManageProspects = false }: {
  ...
  return <div className="space-y-6">
    ...
    {editing !== undefined && <OpportunityDialog opportunity={editing} clients={clients} products={products} managers={managers} onClose={() => setEditing(undefined)} onSaved={() => { setEditing(undefined); router.refresh(); }} />}
...
function OpportunityDialog({ opportunity, clients, products, managers, onClose, onSaved }: {
  ...
  return <Dialog open onOpenChange={(open) => { if (!open) onClose(); }}><DialogContent className="max-h-[92vh] overflow-y-auto sm:max-w-2xl"><form action={save} className="space-y-5"><DialogHeader><DialogTitle>{opportunity ? "Editar oportunidade" : "Nova oportunidade"}</DialogTitle><DialogDescription>Etapa, valor e próximo passo alimentam o funil oficial da AISphere.</DialogDescription></DialogHeader>
    <div className="grid gap-4 sm:grid-cols-2">
      <label className="space-y-1.5 text-sm font-medium sm:col-span-2">Nome<Input name="name" required minLength={3} maxLength={160} defaultValue={opportunity?.name ?? ""} /></label>
      <label className="space-y-1.5 text-sm font-medium">Empresa<select name="client_id" required defaultValue={opportunity?.client_id ?? ""} className="block h-9 w-full rounded-md border bg-background px-3"><option value="" disabled>Selecione</option>{clients.filter((client) => client.active).map((client) => <option key={client.id} value={client.id}>{client.name}{client.client_kind === "prospect" ? " (prospect)" : ""}</option>)}</select></label>
      ...
      <label className="space-y-1.5 text-sm font-medium">Responsável<select name="owner_manager_id" required defaultValue={opportunity?.owner_manager_id ?? managers[0]?.id ?? ""} className="block h-9 w-full rounded-md border bg-background px-3">{managers.map((manager) => <option key={manager.id} value={manager.id}>{manager.name}</option>)}</select></label>
    ...
    <DialogFooter><Button type="button" variant="outline" onClick={onClose}>Cancelar</Button><Button type="submit" disabled={pending || managers.length === 0}>{pending ? "Salvando..." : "Salvar oportunidade"}</Button></DialogFooter>
