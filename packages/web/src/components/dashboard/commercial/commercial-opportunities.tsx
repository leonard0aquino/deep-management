...
import type { Client, ClientContact, CommercialOpportunity, CommercialOpportunityStage, CommercialOpportunityStageEvent, DeepManager, Product } from "@/lib/types/database";
...
function OpportunityDialog({ opportunity, clients, contacts, products, managers, currentManager, currentUserName, onClose, onSaved }: {
  ...
  function save(formData: FormData) {
    ...
    startTransition(async () => {
      ...
      const payload = {
        ...
        owner_manager_id: opportunity?.owner_manager_id ?? currentManager?.id ?? "",
        name: value("name"),
        stage,
  ...
  return <><Dialog open onOpenChange={(open) => { if (!open) onClose(); }}><DialogContent className="max-h-[92vh] overflow-y-auto sm:max-w-2xl"><form action={save} className="space-y-5"><DialogHeader><DialogTitle>{opportunity ? "Editar oportunidade" : "Nova oportunidade"}</DialogTitle><DialogDescription>Empresa, contato, etapa, valor e próximo passo alimentam o funil oficial da AISphere.</DialogDescription></DialogHeader>
    ...
    {!opportunity && !currentManager && <p role="alert" className="text-sm text-destructive">Seu usuário precisa estar vinculado a um gestor DEEP ativo da área Comercial.</p>}
    <DialogFooter><Button type="button" variant="outline" onClick={onClose}>Cancelar</Button><Button type="submit" disabled={pending || (!opportunity && !currentManager)}>{pending ? "Salvando..." : "Salvar oportunidade"}</Button></DialogFooter>
