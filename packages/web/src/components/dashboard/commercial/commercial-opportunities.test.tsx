...
const opportunities: CommercialOpportunity[] = [
  { id: "o1", client_id: "c1", product_id: null, owner_manager_id: "m1", name: "Conta nova", stage: "prospecting", amount: 1000, probability: 10, next_step: "Agendar reunião", next_step_at: "2026-08-05T13:00:00Z", closed_at: null, loss_reason: null, created_by: "u1", updated_by: "u1", created_at: "2026-08-04T10:00:00Z", updated_at: "2026-08-04T10:00:00Z" },
  { id: "o2", client_id: "c1", product_id: "p1", owner_manager_id: "m1", name: "Proposta Legal", stage: "proposal", amount: 5000, probability: 60, next_step: null, next_step_at: null, closed_at: null, loss_reason: null, created_by: "u1", updated_by: "u1", created_at: "2026-08-04T10:00:00Z", updated_at: "2026-08-04T10:00:00Z" },
];
...
describe("CommercialOpportunities", () => {
  ...
  it("filtra a lista pela etapa selecionada", () => {
    render(<CommercialOpportunities opportunities={opportunities} events={[]} clients={clients} products={products} managers={managers} />);
  ...
  it("abre a criação e persiste uma oportunidade estruturada", async () => {
    render(<CommercialOpportunities opportunities={[]} events={[]} clients={clients} products={products} managers={managers} />);
    fireEvent.click(screen.getByRole("button", { name: /Nova oportunidade/i }));
    fireEvent.change(screen.getByLabelText("Nome"), { target: { value: "Venda Legal" } });
