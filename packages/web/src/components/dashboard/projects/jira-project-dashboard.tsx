...
export function JiraProjectDashboard({ project, issues, batches, canImport, referenceDate, projects, selectedProjectKey }: {
  ...
  return <div className="space-y-6">
    ...
    <Card><CardHeader><CardTitle role="heading" aria-level={2}>Por dia</CardTitle><CardDescription>Cards pela data da última atualização recebida do Jira, separados por responsável. É um snapshot de volume, não uma medição de produtividade.</CardDescription></CardHeader><CardContent className="space-y-6">{dashboard.activityByDay.map((activity) => <section key={activity.date} aria-labelledby={`activity-${activity.date}`}><div className="flex items-center justify-between gap-3 border-b pb-2"><h3 id={`activity-${activity.date}`} className="font-semibold">{sourceDate.format(new Date(`${activity.date}T12:00:00Z`))}</h3><span className="text-sm tabular-nums text-muted-foreground">{activity.total} atualizado(s)</span></div><div className="mt-3 grid gap-3 sm:grid-cols-2 xl:grid-cols-3">{activity.assignees.map((assignee) => <div key={assignee.id} className="flex items-center justify-between gap-3 rounded-lg border px-4 py-3"><span className="min-w-0 truncate text-sm font-medium">{assignee.name}</span><span className="shrink-0 text-lg font-semibold tabular-nums">{assignee.total}</span></div>)}</div></section>)}{!dashboard.activityByDay.length && <p className="py-10 text-center text-sm text-muted-foreground">Nenhuma atualização com data para os filtros atuais.</p>}</CardContent></Card>
