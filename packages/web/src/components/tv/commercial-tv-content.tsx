...
import { COMMERCIAL_COCKPIT_KIND_LABEL, type buildCommercialDashboard } from "@/services/commercial-dashboard";
      ...
      <div className="rounded-2xl border border-[var(--tv-border)] bg-[var(--tv-panel)] p-5">
        ...
        <div data-testid="commercial-tv-funnel" className="mt-4 space-y-2">
          {summary.funnel.map((item, index) => <div key={item.key}>
            <div className={`relative mx-auto rounded-xl px-4 py-3 text-white ${FUNNEL_TONE[index]}`} style={{ width: `${100 - index * 9}%` }}><p className="text-xs font-semibold uppercase tracking-[0.14em] opacity-85">{item.label}</p><p className="mt-1 text-3xl font-bold tabular-nums">{item.count}</p></div>
