  ...
  return <div><PageTopbar title="Carteira" description={`${rows.length} clientes acompanhados`} />
    <div className="p-6 sm:p-8"><div className="overflow-x-auto rounded-xl border bg-card">
      <table className="w-full min-w-[1020px] text-sm"><thead className="border-b bg-muted/40 text-left text-xs text-muted-foreground"><tr><th className="px-5 py-3 font-medium">Cliente</th><th className="px-4 py-3 font-medium">Responsável</th><th className="px-4 py-3 font-medium">Produtos</th><th className="px-4 py-3 font-medium">Último contato</th><th className="px-4 py-3 font-medium">Saúde</th><th className="px-4 py-3 font-medium">Status</th><th className="px-5 py-3 text-right font-medium">Ações</th></tr></thead>
        <tbody className="divide-y">{rows.map(({ client, health }) => { const assignmentIds = new Set(data.clientProducts.filter((assignment) => assignment.client_id === client.id).map((assignment) => assignment.id)); const ownerIds = new Set(data.clientProductOwners.filter((owner) => owner.active && assignmentIds.has(owner.client_product_id)).map((owner) => owner.manager_id)); const ownerNames = data.managers.filter((manager) => ownerIds.has(manager.id)).map((manager) => manager.name); return <tr key={client.id} className="hover:bg-muted/25"><td className="px-5 py-4"><p className="font-medium">{client.name}</p><p className="text-xs text-muted-foreground">{client.segment ?? "Sem segmento"}</p></td><td className="px-4 py-4">{ownerNames.length ? ownerNames.join(", ") : <Badge variant="outline" className="border-amber-200 bg-amber-50 text-amber-700">Sem responsável por produto</Badge>}</td><td className="px-4 py-4">{health?.tracked_products ?? 0}</td><td className="px-4 py-4 text-muted-foreground">{health ? formatRecency(health.days_since_last_contact) : "Sem contato"}</td><td className="px-4 py-4">{health ? <><span className={`font-bold ${scoreToneClass(health.score)}`}>{health.score}</span><span className={`ml-2 text-xs ${scoreToneClass(health.score)}`}>{scoreLabel(health.score)}</span></> : "—"}</td><td className="px-4 py-4"><Badge variant={client.active ? "outline" : "secondary"}>{client.active ? "Ativo" : "Inativo"}</Badge></td><td className="px-5 py-4"><div className="flex justify-end gap-2"><Button render={<Link href={`/accounts/${client.id}`} />} nativeButton={false} variant="ghost" size="sm"><ExternalLink /> Ver</Button>{canManage && <EntityEditDialog kind="client" item={client} managers={data.managers} />}</div></td></tr>; })}</tbody>
