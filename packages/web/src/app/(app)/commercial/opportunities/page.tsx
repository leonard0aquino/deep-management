...
import type { Client, CommercialOpportunity, CommercialOpportunityStageEvent, DeepManager, Product, UserProfile } from "@/lib/types/database";

export default async function CommercialOpportunitiesPage() {
  const context = await requireAccess("commercial");
  const supabase = await createClient();
  const admin = createAdminClient();
  const [opportunities, events, clients, products, managers, profiles] = await Promise.all([
  ...
  return <div><PageTopbar title="Funil Comercial" description="Oportunidades, etapas e próximos passos com histórico auditável" /><div className="p-6 sm:p-8"><CommercialOpportunities opportunities={opportunities.data ?? []} events={events.data ?? []} clients={clients.data ?? []} products={products.data ?? []} managers={visibleManagers} canManageProspects={canManageOperations(context.role)} /></div></div>;
}
