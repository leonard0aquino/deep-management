...
drop index if exists public.client_contacts_role_owner_idx;

alter table public.client_contacts
  drop column if exists owner_manager_id,
  drop column if exists relationship_role;

drop type if exists public.stakeholder_relationship_role;
...
select
  ...
  cc.email, cc.phone, cc.influence, cc.photo_url, cc.reports_to_contact_id,
  agg.last_contact, agg.interaction_count,
...
grant select on table public.stakeholder_health to authenticated, service_role;
grant references, trigger, truncate on table public.client_contacts to authenticated;
